...
export default function drawThinBorders(gl, { path, normals, thickness }) {
    ...
    gl.uniform3f(colorUniformLocation, 0, 0, 1);
    ...
    const data = []
    for (let i = 0, qty = path.length; i < qty; i++) {
      data.push(...path[i], ...normals[i][0], normals[i][1])
    }
    for (let i = 0, qty = path.length; i < qty; i++) {
      data.push(...path[i], ...normals[i][0], -normals[i][1])
    }
    ...
    gl.drawArrays(gl.LINE_STRIP, path.length, path.length);
